...
- **Threaded dispatch:** how the interpreter runs bytecode — each opcode's handler ends by tail-jumping directly into the next handler. There is no central `switch` and no single dispatcher control returns to; the fetch-and-dispatch step lives at the tail of every handler instead, so each gets its own branch-prediction history.
