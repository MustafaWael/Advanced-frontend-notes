...
- **"Stable shapes" is really "give the compiler a fact instead of a hint."** An inline cache is the engine guessing from history. Consistent shapes do not make the guess faster — they make it *unnecessary*, which is strictly better, because a correct guess still costs a check and a wrong one costs a bail-out. The same move appears everywhere in [[32 - Compilation and Machine Foundations/00 - Compilation and Machine Foundations MOC|module 32]]: a direct call beats a correctly-speculated indirect one, and a build-time decision beats a runtime one.
...
| **Why "stable shapes" matters** | Two reasons, and give both: fixed-offset loads with good locality instead of hash probes, *and* a monomorphic site the optimizer can specialize. Best framing: it makes the engine's guess unnecessary rather than faster. |
...
- Treating an inline cache hit as free. A correct guess still costs a shape check; consistent shapes are better because they remove the guess, not because they speed it up.
