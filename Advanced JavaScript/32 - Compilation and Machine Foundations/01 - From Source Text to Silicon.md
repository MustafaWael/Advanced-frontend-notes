...
`i32.add` is a *different opcode* from `f64.add`. The type is baked into the instruction, so no *type* has to be discovered at runtime — the structural reason Wasm needs no inline caches and no type-feedback warm-up. It does not follow that nothing is profiled: in current V8, Liftoff emits code to update a feedback vector at each call site, and hot functions still tier up to the optimizing compiler. Static types remove type speculation, not profiling. See [[32 - Compilation and Machine Foundations/11 - Wasm Speculation and Deopt|Wasm Speculation and Deopt]].
...
The Wasm module's instructions carry their types, so V8's Liftoff compiler emits machine code in one pass at load — no type-feedback warm-up before useful work starts, and no type-driven bailout. (Optimized Wasm code still arrives later via tier-up, and V8 can still deopt on non-type speculation; the point here is that the *first* run is already fast.) Same algorithm, a layer lower.
