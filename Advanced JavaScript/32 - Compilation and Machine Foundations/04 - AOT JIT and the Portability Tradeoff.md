...
3. <details><summary>Wasm is portable like JVM bytecode but needs no <em>type</em>-feedback warm-up. What makes that possible, and what is still deferred?</summary>Its instructions are statically typed and validated in a single linear pass, so the engine can compile straight to machine code on arrival with no need to observe types first — the first run is already fast. What is still deferred is optimization: Liftoff's output is unoptimized, hot functions still tier up to the optimizing compiler on a hotness heuristic, and current V8 profiles indirect-call targets and can deopt on that non-type speculation. So "no warm-up" is true of types and false of peak performance.</details>
