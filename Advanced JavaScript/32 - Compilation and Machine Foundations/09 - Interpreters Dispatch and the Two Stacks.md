...
V8 uses **indirect threaded dispatch**: each bytecode handler is separately pre-compiled machine code, and ends with a tail call that reads the next opcode and jumps straight into that handler. No loop, no central `switch`, no return to a parent frame. The reason is branch prediction — one central dispatch branch would mispredict on nearly every bytecode, whereas each handler's own dispatch site keeps its own prediction history. Full mechanism in [[02 - JavaScript Runtime Foundations/09 - Bytecode Dispatch and Tier-Up|Bytecode Dispatch and Tier-Up]].
